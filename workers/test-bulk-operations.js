...
const WORKER_URL = process.env.WORKER_URL || 'https://woood-production.leander-4e0.workers.dev';
...
async function testBulkOperations() {
  ...
  try {
    ...
    const response = await fetch(`${WORKER_URL}/api/experience-center/bulk-test`, {
      method: 'POST',
      headers: {
    ...
    if (result.summary) {
      ...
      console.log(`   Successful Products: ${result.summary.successfulProducts}`);
      console.log(`   Failed Products: ${result.summary.failedProducts}`);
      console.log(`   EAN Matches: ${result.summary.eanMatches || 'N/A'}`);
      console.log(`   Total Products: ${result.summary.totalProducts || 'N/A'}`);
    ...
    if (result.result) {
      ...
      if (result.result.errors && result.result.errors.length > 0) {
        console.log('');
        console.log('❌ Errors:');
        result.result.errors.slice(0, 3).forEach((/** @type {any} */ error, /** @type {number} */ index) => {
          console.log(`   ${index + 1}. ${String(error)}`);
        });
        if (result.result.errors.length > 3) {
          console.log(`   ... and ${result.result.errors.length - 3} more errors`);
        }
      }
    }
