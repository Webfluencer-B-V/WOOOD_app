...
async function upsertShopMetafield(
	env: Env,
	dealers: any[],
	shop: string,
): Promise<any> {
	...
	async fetch(
		...
		const corsHeaders = {
			...
			"Access-Control-Allow-Headers":
				"Content-Type, Authorization, X-Shopify-Hmac-Sha256, X-Shopify-Topic, X-Shopify-Shop-Domain, x-api-key",
		...
		try {
			...
			switch (true) {
				...
				case path === "/api/experience-center/trigger" &&
					request.method === "POST":
					response = await handleExperienceCenterTrigger(request, env);
					break;

				case path === "/api/experience-center/status" &&
					request.method === "GET":
					response = await handleExperienceCenterStatus(request, env);
					break;

				case path === "/api/experience-center/bulk-test" &&
					request.method === "POST":
		...
		try {
			...
			if (env.STORE_LOCATOR_STATUS) {
				await env.STORE_LOCATOR_STATUS.put(
					STORE_LOCATOR_STATUS_KEY,
					JSON.stringify(status),
				);
			}
			console.log(
				`✅ Store locator update completed: ${dealers.length} dealers processed for ${successfulShops}/${installedShops.length} shops`,
			);
		} catch (error: any) {
		...
		try {
			...
			if (env.EXPERIENCE_CENTER_STATUS) {
				await env.EXPERIENCE_CENTER_STATUS.put(
					EXPERIENCE_CENTER_STATUS_KEY,
					JSON.stringify(result),
				);
			}
			console.log(
				`✅ Experience center update completed: ${result.summary?.successfulProducts || 0} successful, ${result.summary?.failedProducts || 0} failed`,
			);
		} catch (error: any) {
...
async function handleInstall(request: Request, env: Env): Promise<Response> {
	...
	const scopes =
		"read_products,write_products,write_checkouts,write_delivery_customizations,write_orders";
	const redirectUri = `${env.SHOPIFY_APP_URL}/auth/callback`;

	// Store state temporarily
	if (env.WOOOD_KV) {
		await env.WOOOD_KV.put(`oauth_state:${state}`, shop, {
			expirationTtl: 600,
		});
	}
		...
		if (dates.length === 0) {
			if (env.ENABLE_MOCK_FALLBACK === "true") {
				...
				if (response.ok) {
					...
					dates = Array.isArray(apiData)
						? apiData.map((item: any) => ({
								date: item.date,
								displayName:
									item.displayName || formatDateInDutch(new Date(item.date)),
							}))
						: [];
				} else {
					dates = generateMockDates();
...
async function handleOrderWebhook(
	...
	try {
		...
		const metafields = transformNoteAttributesToMetafields(
		...
		if (metafields.length === 0) {
			console.log(
				`No delivery metafields to create for order #${orderData.id}`,
			);
			return new Response(
				JSON.stringify({ success: true, metafieldsCreated: 0 }),
				{
					headers: { "Content-Type": "application/json" },
				},
			);
...
async function handleTestWebhooks(
	request: Request,
	env: Env,
): Promise<Response> {
...
async function handleStoreLocatorUpsert(
	request: Request,
	env: Env,
): Promise<Response> {
	try {
...
async function handleStoreLocatorStatus(
	request: Request,
	env: Env,
): Promise<Response> {
	if (env.STORE_LOCATOR_STATUS) {
	...
	return new Response(
		JSON.stringify({ success: false, message: "No status available" }),
		{ headers: { "Content-Type": "application/json" } },
	);
}

// Experience center functions - OPTIMIZED WITH BULK OPERATIONS
async function fetchExperienceCenterData(
	env: Env,
...
async function processExperienceCenterWithBulkOperations(
	env: Env,
	shop: string,
	...
	console.log(
		`🚀 Starting bulk operations for ${shop} with ${availableEans.size} available EANs`,
	);
  ...
  `;
    ...
    `;
		...
		console.log(
			`📊 Bulk operation status: ${operationStatus}, objects: ${bulkOperation.objectCount || 0}`,
		);

		if (operationStatus === "FAILED") {
			throw new Error(
				`Bulk operation failed: ${bulkOperation.errorCode || "Unknown error"}`,
			);
		}
	...
	console.log(
		`✅ Bulk operation completed, downloading data from: ${downloadUrl}`,
	);
	...
	const variants = new Map<
		string,
		{ id: string; barcode: string | null; productId: string }
	>();
	...
	console.log(
		`📊 Parsed ${products.size} products and ${variants.size} variants`,
	);
	...
	console.log(
		`📊 Prepared ${metafieldsToUpdate.length} metafield updates (${processedProducts} processed, ${skippedProducts} skipped, ${eanMatches} EAN matches)`,
	);
	...
	for (let i = 0; i < metafieldsToUpdate.length; i += batchSize) {
		...
		try {
			console.log(
				`🔄 Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(metafieldsToUpdate.length / batchSize)} (${batch.length} products)`,
			);
	...
	console.log(
		`🎯 Bulk operations completed for ${shop}: ${totalSuccessful} successful, ${totalFailed} failed, ${eanMatches} EAN matches out of ${availableEans.size} available`,
	);
...
async function setProductExperienceCenterMetafieldsBulk(
	env: Env,
	shop: string,
  ...
  `;
	...
	const errors = userErrors.map(
		(error: any) => `${error.field}: ${error.message}`,
	);
...
async function processExperienceCenterUpdateAllShops(env: Env): Promise<any> {
	...
	const experienceCenterData = await fetchExperienceCenterData(env);
	console.log(
		`📊 Fetched ${experienceCenterData.total} Experience Center products from WOOOD API`,
	);
	...
	console.log(
		`🏪 Processing ${installedShops.length} shops with ${availableEans.size} available EANs`,
	);
	...
	for (let i = 0; i < installedShops.length; i++) {
		...
		if (elapsedTime > maxExecutionTime) {
			console.log(
				`⏰ Time limit approaching (${Math.round(elapsedTime / 1000)}s elapsed), stopping processing`,
			);
		...
		try {
			console.log(
				`🏪 Processing shop ${i + 1}/${installedShops.length}: ${shop} (${Math.round(elapsedTime / 1000)}s elapsed)`,
			);
			...
			totalFailed += failed;
			console.log(
				`✅ Experience center update completed for ${shop}: ${successful} successful, ${failed} failed, ${eanMatches} EAN matches`,
			);
	...
	console.log(
		`🎯 All shops processed in ${totalTime}s: ${successfulShops}/${installedShops.length} successful`,
	);
...
async function handleExperienceCenterTrigger(
	request: Request,
	env: Env,
): Promise<Response> {
	try {
		...
		return new Response(
			JSON.stringify({
				...
				results: [
					{
						shop: env.SHOPIFY_STORE_URL?.replace("https://", "").replace(
							"http://",
							"",
						),
						success: false,
...
async function handleExperienceCenterStatus(
	request: Request,
	env: Env,
): Promise<Response> {
	try {
...
async function handleExperienceCenterBulkTest(
	request: Request,
	env: Env,
): Promise<Response> {
	try {
		...
		const shop =
			body.shop ||
			env.SHOPIFY_STORE_URL?.replace("https://", "").replace("http://", "");
...
async function registerWebhooks(
	...
	const webhooks = [
		{
			topic: "orders/paid",
			address: `${env.SHOPIFY_APP_URL}/api/webhooks/orders`,
		},
		{
			topic: "orders/create",
			address: `${env.SHOPIFY_APP_URL}/api/webhooks/orders`,
	...
	for (const webhook of webhooks) {
		try {
			console.log(
				`Registering webhook: ${webhook.topic} -> ${webhook.address}`,
			);
...
function transformNoteAttributesToMetafields(
...
async function createOrderMetafields(
	...
	for (const metafield of metafields) {
		try {
			console.log(
				`Creating metafield: ${metafield.namespace}.${metafield.key} = ${metafield.value}`,
			);

			const response = await fetch(
				`https://${shop}/admin/api/2024-10/orders/${orderId}/metafields.json`,
				{
					method: "POST",
					headers: {
						"X-Shopify-Access-Token": accessToken,
			...
			if (response.ok) {
				const result = (await response.json()) as any;
				console.log(
					`✅ Successfully created metafield: ${metafield.namespace}.${metafield.key}`,
					{
				...
				const errorText = await response.text();
				console.error(
					`❌ Failed to create metafield: ${metafield.namespace}.${metafield.key}`,
					{
						status: response.status,
						error: errorText,
						metafield: metafield,
			...
			console.error(
				`❌ Exception creating metafield: ${metafield.namespace}.${metafield.key}`,
				{
					error: (error as Error).message,
					metafield: metafield,
				},
